...
使用方式：
  ...
  # 形态 A（具名，默认）
  python mode_cli/run_cli.py --mode named --question "宁德现在天气如何？"
  # 形态 B（通用 bash）
  python mode_cli/run_cli.py --mode bash --question "宁德现在天气如何？"
  # 内置示例
...
def run(client, model: str, question: str, mode: str, verbose: bool = True) -> dict:
    """
    多轮循环：提问 -> 模型输出 tool_call -> 执行命令 -> 回填 -> 再请求 ...
    ...
    for round_i in range(MAX_ROUNDS):
        ...
        for tc in msg.tool_calls:
            ...
            if verbose:
                print(f"  -> [{mode}] {tc.function.name}({args})  [round {rounds}]")
            try:
