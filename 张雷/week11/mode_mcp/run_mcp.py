"""
run_mcp.py - 方式二：MCP Host（连接 Server，多轮循环调用）
...
教学重点：
  ...
  3. 多轮循环代码和 run_function_call.py 几乎一样--差异只在"工具从哪来/怎么执行"
     · Function Call：手写 schema + 直接调后端函数
     · MCP：发现 schema + 通过 call_tool 跨进程调用 Server
  4. AsyncExitStack 管理 Server 子进程的生命周期

使用方式：
  python mode_mcp/run_mcp.py --question "宁德现在天气如何？"
  python mode_mcp/run_mcp.py --demo

依赖：
  pip install mcp openai httpx
  环境变量：DEEPSEEK_API_KEY（默认 LLM；--provider dashscope 切 qwen-plus）

MCP 三角关系：
  Host（本文件）= 连接管理 + 工具路由 + LLM 多轮循环
  Client        = ClientSession，与 Server 的会话
...
def build_server_configs() -> dict[str, StdioServerParameters]:
    # 自写 Server，用项目内 Python 脚本启动，stdio 通信
...
async def run(client, model: str, question: str,
    ...
    """
    多轮循环：提问 -> 模型输出 tool_call -> 路由到 Server 执行 -> 回填 -> 再请求 ...
    ...
    for round_i in range(MAX_ROUNDS):
        ...
        for tc in msg.tool_calls:
            ...
            if verbose:
                print(f"  -> [mcp] {name}({args})  [round {rounds}]")
