...
使用方式：
  python compare.py                 # 跑内置 3 个天气问题
  python compare.py --questions "宁德现在天气如何？" "北京和上海哪个更热？"
  python compare.py --provider dashscope

环境变量：DEEPSEEK_API_KEY（默认 LLM）/ DASHSCOPE_API_KEY（备选 LLM qwen-plus）
...
def write_markdown(rows: list[dict], questions: list[str], provider: str, path: Path):
    ...
    lines += [
        ...
        "CLI 与模型完全无关。",
        "- **多轮循环**：天气条件题（如『若气温低于20度则查A城，否则查B城』）迫使模型先查一个城市、"
        "看到结果再决定查哪个，四方式都应跨多轮完成。",
        "",
        "## 各方式原始回答",
        "",
    ]
