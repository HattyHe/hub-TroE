...
pip install -e .
```
这会把 `fincli` 注册到 PATH（`pyproject.toml` 的 `[project.scripts]`），之后在任意目录都能像 `git`/`ls` 一样直接敲 `fincli weather ...`。方式三的 CLI 形态依赖它；不装也能用（自动退回 `python mode_cli/cli/main.py`，只是命令不"漂亮"）。
...
```
**内部流程**：
1. 手写 `TOOLS_SCHEMA`（`get_weather` 工具的 JSON Schema）
...
**预期输出**：先打印 `-> [tool] get_weather({...})` 与结果预览，再打印最终回答。
...
2. Server `initialize()` 握手 -> `list_tools()` 发现工具，打印 `✓ [weather] get_weather`；同时把 MCP `inputSchema` 转 OpenAI `parameters`（一次走完连接+发现+转 schema）
3. LLM 多轮循环：`tool_call` -> 查 `tool_registry` 路由到对应 `ClientSession` -> `session.call_tool()` -> 结果回填
4. 模型生成最终回答

**预期输出**：stderr 打印连接日志 + `共 1 个工具就绪`；stdout 打印工具调用与最终回答。
...
> 没装 fincli？用 `python mode_cli/cli/main.py weather --city 宁德` 或 `python -m mode_cli.cli.main weather --city 宁德` 也能跑，效果一样。
...
```bash
fincli weather --city 宁德 | head -20
```
...
```
LLM 调 `run_cli(command='weather', args={city: '宁德'})`，host 按 `NAMED_COMMANDS` 白名单拼出 `fincli weather ...` 执行。`command` 是 enum，模型只能选预批准的命令--安全可控，但每加一个命令要改代码。

**形态 B（通用 `run_bash`，沙箱，更灵活）**：
```bash
python mode_cli/run_cli.py --mode bash -q "宁德现在天气如何？如果气温低于20度再查哈尔滨。"
```
LLM 自己拼完整 shell 命令字符串（如 `fincli weather --city 宁德`），host 经 `sandbox_check` 后 `subprocess.run(shell=True)` 执行。最灵活也最危险，靠沙箱兜底。
...
MCP Server 的 tool 函数与导入的后端函数同名导致递归。本项目已用 `as` 别名（`get_weather as _get_weather`）修复，若你新增 Server 工具，注意别让 tool 函数名与 import 的后端函数名相同。
