...
- `src/` - 共享业务逻辑：`weather_backend.py`（Open-Meteo 天气，含地名歧义处理）。
...
- 工具集只有一个 `get_weather`，故循环无需区分工具类型（早期版本有「天气门控」让 RAG 工具单轮、天气多轮；RAG 删除后门控变死代码已移除，循环简化为纯多轮）。
...
- **MCP Server stdout 是协议通道**：`mode_mcp/servers/*.py` 里所有日志必须写 `sys.stderr`，混进 stdout 会破坏 JSON-RPC 连接。后端函数用 `as` 别名导入（如 `get_weather as _get_weather`），否则 `@mcp.tool()` 同名函数会遮蔽后端、递归调自己。
