...
class multiClassRnn(nn.Module):
    ...
    def __init__(self, vocab_size, embed_dim=EMBED_DIM, hidden_dim=HIDDEN_DIM, dropout=0.3):
        ...
        # 当使用双向LSTM时，输出的维度会变成 2 * hidden_dim（因为正向和反向的输出会拼接）
        # 全连接层：负责分类决策
    ...
    def forward(self, x, y=None):
        ...
        前向传播，计算损失
        """
        # 嵌入前：x 形状为 (batch_size, seq_len)，如 (64, 32)
        # 嵌入后：x 形状为 (batch_size, seq_len, embed_dim)，如 (64, 32, 64)

        x = self.embedding(x) #输出batch_size, seq_lenth, embed_dim=64
        e, _ = self.lstm(x) #输出batch_size, seq_lenth, hidden_dim*2=128
